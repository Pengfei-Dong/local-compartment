...
Args <- commandArgs(T)
if(length(Args)!=5){
  cat(paste0('usage Rscript call_local_compartment.R bin_ord ',
             'resolution interaction.matrix all_valid out_prefix'),'\n')
  quit('no',0,F)
}
bin_ord_fh <- Args[1] 
...
all_valid <- Args[4]
#tomato_hic/hic_results/data/tomato3/tomato3_allValidPairs
out_prefix <- Args[5] # tomato3
